...
const HomeMapsNew: React.FC<HomeMapsNewProps> = ({ searchedPlace }) => {
  ...
  const [markerPosition, setMarkerPosition] = useState<{ lat: number; lng: number } | null>(null);
  const [restaurants, setRestaurants] = useState<any[]>([]); // Store restaurants with extra data like images and IDs
  const [savedRestaurants, setSavedRestaurants] = useState<any[]>([]); // Store saved restaurants
  ...
  const fetchRestaurants = (lat: number, lng: number) => {
    if (typeof google !== "undefined" && google.maps && google.maps.places) {
      ...
      service.nearbySearch(request, (results, status) => {
        if (status === google.maps.places.PlacesServiceStatus.OK && results) {
          const restaurantList = results.slice(0, 3).map((place, index) => ({
            id: index, // Assign an id for each restaurant
            name: place.name || "Unknown Name",
            address: place.vicinity || "Unknown Address",
            image: place.photos ? place.photos[0].getUrl({ maxWidth: 100, maxHeight: 100 }) : null, // Get an image if available
  ...
  const saveRestaurant = (restaurant: any) => {
    setSavedRestaurants((prevSaved) => [...prevSaved, restaurant]);
  };
    ...
    <div style={{ display: "flex" }}>
      ...
      <div style={listStyle}>
        ...
        {restaurants.length > 0 ? (
          <ul>
            {restaurants.map((restaurant) => (
              <li key={restaurant.id} style={{ display: "flex", marginBottom: "15px", padding: "10px", backgroundColor: "#fff", borderRadius: "8px", boxShadow: "0 2px 5px rgba(0,0,0,0.1)" }}>
                {/* Left part with restaurant image */}
                <div style={{ marginRight: "15px" }}>
                  {restaurant.image ? (
                    <img src={restaurant.image} alt={restaurant.name} style={{ width: "50px", height: "50px", borderRadius: "50%" }} />
                  ) : (
                ...
                <div>
                  ...
                  <button
                    ...
                    style={{ backgroundColor: "#007bff", color: "#fff", padding: "5px 10px", borderRadius: "5px", border: "none", cursor: "pointer" }}
                  >
                    Save
                  </button>
                </div>
